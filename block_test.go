...
func TestBlock(t *testing.T) {
	b := vtcutil.NewBlock(&Block100000)
...
func TestNewBlockFromBytes(t *testing.T) {
	...
	b, err := vtcutil.NewBlockFromBytes(block100000Bytes)
	if err != nil {
...
func TestNewBlockFromBlockAndBytes(t *testing.T) {
	...
	// Create a new block from the serialized bytes.
	b := vtcutil.NewBlockFromBlockAndBytes(&Block100000, block100000Bytes)
...
func TestBlockErrors(t *testing.T) {
	...
	testErr := vtcutil.OutOfRangeError(wantErr)
	...
	_, err = vtcutil.NewBlockFromBytes(shortBytes)
	if err != io.EOF {
	...
	_, err = b.TxHash(-1)
	if _, ok := err.(vtcutil.OutOfRangeError); !ok {
		t.Errorf("TxHash: wrong error - got: %v <%T>, "+
			"want: <%T>", err, err, vtcutil.OutOfRangeError(""))
	}
