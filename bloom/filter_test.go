...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := vtcutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(vtcutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := vtcutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := vtcutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
