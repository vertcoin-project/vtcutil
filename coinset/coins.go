...
func (cs *CoinSet) TotalValue() (value vtcutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue vtcutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue vtcutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount vtcutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue vtcutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue vtcutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue vtcutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue vtcutil.Amount, coins []Coin) (Coins, error) {
...
// vtcutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() vtcutil.Amount {
	return vtcutil.Amount(c.txOut().Value)
}
